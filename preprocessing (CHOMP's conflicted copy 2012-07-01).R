...
rownames(reg_benchmarks$nationality) = c("Dutch", "not Dutch")
...
tmp <- tmp[-2:-8]
tmp[2] = c('Other')
reg_benchmarks$originself[2,] = sum(reg_benchmarks$originself[2:7,])
reg_benchmarks$originself <- as.data.frame(reg_benchmarks$originself[-3:-8,])
rownames(reg_benchmarks$originself) = tmp
names(reg_benchmarks$originself) = c("Percent")

tmp <- rownames(reg_benchmarks$originfather)
tmp <- tmp[-2:-8]
tmp[2] = c('Other')
reg_benchmarks$originfather[2,] = sum(reg_benchmarks$originfather[2:7,])
reg_benchmarks$originfather <- as.data.frame(reg_benchmarks$originfather[-3:-8,])
rownames(reg_benchmarks$originfather) = tmp
names(reg_benchmarks$originfather) = c("Percent")

tmp <- rownames(reg_benchmarks$originmother)
tmp <- tmp[-2:-8]
tmp[2] = c('Other')
reg_benchmarks$originmother[2,] = sum(reg_benchmarks$originmother[2:7,])
reg_benchmarks$originmother <- as.data.frame(reg_benchmarks$originmother[-3:-8,])
